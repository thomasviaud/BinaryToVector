...
t_ledge* add_edge(t_vertex* src, t_vertex* dest, t_ledge* ledge, char ename){
	t_ledge* new;
	new=create_ledge();
	new->val=set_edge(src,dest,ename);
	new->next=ledge;
	return new;
}
...
void print_edge(t_edge* edge){
	printf("Arc %d : du sommet %d au sommet %d\n", edge->name,edge->src->name, edge->dest->name);
}

void print_lvertex(t_lvertex* lvertex){
	if(lvertex->next==NULL) return;
	print_vertex(lvertex->val);
	print_lvertex(lvertex->next);
	return;
}
void print_vertex(t_vertex* vertex){
	printf("Sommet %d en [%d][%d] de type %d\n",vertex->name,vertex->posx,vertex->posy,vertex->point.type);
}